...
class OrderLogic
{
    public static function create_transaction($order, $received_by=false, $status = null)
    {
        ...
        if($order->free_delivery_by == 'admin')
        {
            ...
            Helpers::expenseCreate($order->original_delivery_charge,'free_delivery',now(),$order->free_delivery_by,$order->id);
        }
        // free delivery by restaurant
        if($order->free_delivery_by == 'vendor')
        {
            $restaurant_subsidy = $order->original_delivery_charge;
            Helpers::expenseCreate($order->original_delivery_charge,'free_delivery',now(),$order->free_delivery_by,$order->id,$order->restaurant->id);
        }
        // coupon discount by Admin
        if($order->coupon_created_by == 'admin')
        {
            $admin_coupon_discount_subsidy = $order->coupon_discount_amount;
            Helpers::expenseCreate($admin_coupon_discount_subsidy,'coupon_discount',now(),$order->coupon_created_by,$order->id);
        }
        // 1st order discount by Admin
        if($order->ref_bonus_amount > 0)
        {
            $ref_bonus_amount = $order->ref_bonus_amount;
            Helpers::expenseCreate($ref_bonus_amount,'referral_discount',now(),'admin',$order->id);
        }
        ...
        if($order->coupon_created_by == 'vendor')
        {
            ...
            Helpers::expenseCreate($restaurant_coupon_discount_subsidy,'coupon_discount',now(),$order->coupon_created_by,$order->id,$order->restaurant->id);
        }
        ...
        if($order->restaurant_discount_amount > 0  && $order->discount_on_product_by == 'vendor')
        {
            if($restaurant->restaurant_model == 'subscription' && isset($rest_sub)){
                ...
                Helpers::expenseCreate($restaurant_d_amount,'discount_on_product',now(),'vendor',$order->id,$order->restaurant->id);
            } else{
                ...
                Helpers::expenseCreate($restaurant_d_amount,'discount_on_product',now(),'vendor',$order->id,$order->restaurant->id);
                Helpers::expenseCreate($amount_admin,'discount_on_product',now(),'admin',$order->id);
            }
        }
        ...
        if($order->restaurant_discount_amount > 0  && $order->discount_on_product_by == 'admin')
        {
            ...
            Helpers::expenseCreate($restaurant_discount_amount,'discount_on_product',now(),'admin',$order->id,);
        }
           ...
           (($restaurant->restaurant_model == 'subscription' &&  $rest_sub->self_delivery == 0) || ($restaurant->restaurant_model != 'subscription' && $restaurant->self_delivery_system == 0))
            ...
            try
            {
                ...
                if(isset($restaurant_over_flow) ){
                    self::create_account_transaction_for_collect_cash($vendorWallet->collected_cash , 'restaurant' , $order->restaurant->vendor->id , $order->order_amount - $order->partially_paid_amount ,$order->id);
                }
                if(isset($dm_over_flow)){
                    self::create_account_transaction_for_collect_cash($dmWallet->collected_cash , 'deliveryman' , $order->delivery_man_id ,  $order->order_amount - $order->partially_paid_amount , $order->id);
                }



                self::update_unpaid_order_payment( $order->id, $order->payment_method);
                ...
                if($order->is_guest  == 0){
                    ...
                    if($order->user_id) CustomerLogic::create_loyalty_point_transaction($order->user_id, $order->order_amount, 'order_place', $order->id);
                }
            }
            catch(\Exception $exception)
            {
    ...
    public static function cashbackToWallet($order){
        ...
        if($refer_wallet_transaction != false){
            Helpers::expenseCreate($order->cashback_history->calculated_amount,'CashBack',now(),'admin',$order->id);
