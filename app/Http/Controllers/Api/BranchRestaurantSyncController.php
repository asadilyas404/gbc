    ...
    public function getData(Request $request)
    {
        try {
            ...
            $branches = DB::connection('oracle')
                ...
                ->when(!$snapshot ? $cursorMap['branches'] : null, function ($query, Carbon $cursor) {
            ...
            $restaurants = DB::connection('oracle')
                ->table('restaurants')
                ->when(!$snapshot ? $cursorMap['restaurants'] : null, function ($query, Carbon $cursor) {
                    $query->where(function ($subQuery) use ($cursor) {
                        $subQuery->where('updated_at', '>', $cursor->toDateTimeString())
                            ->orWhereNull('updated_at');
                    });
            ...
            $partners = DB::connection('oracle')
                ...
                ->when(!$snapshot ? $cursorMap['partners'] : null, function ($query, Carbon $cursor) {
                    $query->where(function ($subQuery) use ($cursor) {
                        $subQuery->where('updated_at', '>', $cursor->toDateTimeString())
                            ->orWhereNull('updated_at');
                    });
            ...
            $banks = DB::connection('oracle')
                ->table('tbl_defi_bank')
                ->when(!$snapshot ? ($cursorMap['banks'] ?? null) : null, function ($query, Carbon $cursor) {
                    $query->where(function ($subQuery) use ($cursor) {
                        $subQuery->where('updated_at', '>', $cursor->toDateTimeString())
                            ->orWhereNull('updated_at');
                    });
            ...
            $vendors = DB::connection('oracle')
                ->table('vendors')
                ->when(!$snapshot ? ($cursorMap['vendors'] ?? null) : null, function ($query, Carbon $cursor) {
                    $query->where(function ($subQuery) use ($cursor) {
                        $subQuery->where('updated_at', '>', $cursor->toDateTimeString())
                            ->orWhereNull('updated_at');
                    });
            ...
            $posDiscountTypes = DB::connection('oracle')
                ...
                ->when(!$snapshot ? ($cursorMap['pos_discount_types'] ?? null) : null, function ($query, Carbon $cursor) {
                    $query->where(function ($subQuery) use ($cursor) {
                        $subQuery->where('updated_at', '>', $cursor->toDateTimeString())
                            ->orWhereNull('updated_at');
                    });
