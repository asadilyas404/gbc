...
class DashboardController extends Controller
{
    public function dashboard(Request $request)
    {
        ...
        $restaurant_earnings = OrderTransaction::NotRefunded()
        ->where(['vendor_id' => Helpers::get_vendor_id()])
        ->selectRaw('NVL(SUM(restaurant_amount), 0) as earning')
        ->selectRaw('NVL(SUM(admin_commission + admin_expense), 0) as commission')
        ->selectRaw('NVL(SUM(delivery_charge), 0) as delivery_earning')
