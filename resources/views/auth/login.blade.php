...
<main id="content" role="main" class="main auth-bg">
    ...
    <div class="d-flex flex-wrap align-items-center justify-content-between">
        <div class="auth-content">
            <div class="content">
                <h2 class="title text-uppercase">{{__('messages.welcome_to')}} {{ $app_name??'STACKFOOD' }}</h2>
                <p>
                    {{__('Manage_your_app_&_website_easily')}}
                </p>
            </div>
        </div>
            ...
            <div class="auth-wrapper-body auth-form-appear">
                ...
                <div class="text-center">
                    <div class="auth-header mb-5">
                        <h2 class="signin-txt">{{ __('messages.Signin_To_Your_Panel')}}</h2>
                    </div>
                </div>
                <!-- Content -->
                ...
                <form class="login_form" action="{{route('login_post')}}" method="post" id="form-id">
                    ...
                    <div class="js-form-message form-group mb-2">
                        <label class="form-label text-capitalize" for="signinSrEmail">{{__('messages.your_email')}}</label>
                    ...
                    <!-- forget password -->
                        <div class="form-group {{ $role == 'admin' ? '' : 'd-none' }}"  id="forget-password">
                            <div class="custom-control">
                                <span type="button" data-toggle="modal" data-target="#forgetPassModal">{{ __('Forget_Password?') }}</span>
                            </div>
                        </div>
                        <div class="form-group {{ $role == 'vendor' ? '' : 'd-none' }}"  id="forget-password1">
                            <div class="custom-control">
                                <span type="button" data-toggle="modal" data-target="#forgetPassModal1">{{ __('Forget_Password?') }}</span>
                            </div>
                        </div>
                    </div>
                    <!-- End forget password -->
...
{{--                                       id="custome_recaptcha" required placeholder="{{__('Enter recaptcha value')}}" autocomplete="off" value="{{env('APP_MODE')=='dev'? session('six_captcha'):''}}">--}}
{{--                            </div>--}}
...
{{--                    @endif--}}
                    ...
                    <button type="submit" class="btn btn-lg btn-block btn-primary" id="signInBtn">{{__('messages.sign_in')}}</button>
                </form>
                <!-- End Form -->
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="forget-pass-content">
              ...
              <!-- After Succeed -->
              <h4>
                  {{ __('Send_Mail_to_Your_Email_?') }}
              </h4>
              <p>
                  {{ __('A_mail_will_be_send_to_your_registered_email_with_a_link_to_change_passowrd') }}
              </p>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="forget-pass-content">
              ...
              <!-- After Succeed -->
              <h4>
                  {{ __('messages.Send_Mail_to_Your_Email_?') }}
              </h4>
              <form class="" action="{{ route('vendor-reset-password') }}" method="post">
                  @csrf

                  <input type="email" name="email" id="" class="form-control" required>
                  <button type="submit" class="btn btn-lg btn-block btn--primary mt-3">{{ __('messages.Send_Mail') }}</button>
              </form>
          </div>
        </div>
      </div>
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <div class="forget-pass-content">
                ...
                <h4>
                  {{ __('A_mail_has_been_sent_to_your_registered_email') }}!
                </h4>
                <p>
                  {{ __('Click_the_link_in_the_mail_description_to_change_password') }}
                </p>
            </div>
          </div>
        </div>
