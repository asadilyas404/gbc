...
<div class="receipt">
        ...
        @foreach ($order->details as $detail)
            ...
            @if (($detail->food_id || $detail->campaign == null) && trim($detail->is_deleted) != 'Y')
                ...
                <tr>
                    ...
                    <td class="price">{{ number_format($detail->price, 3) }}</td>
                    <td class="total">{{ number_format($itemTotal, 3) }}</td>
                </tr>
                            ...
                            @foreach ($variation['addons'] as $addon)
                                ...
                                <tr>
                                    <td colspan="4" class="addon-line">
                                        ...
                                        +{{ number_format($addonPrice, 3) }} <span class="currency">(ر.ع)</span>
                                    </td>
                                </tr>
                ...
                <tr>
                    <td colspan="4" class="item-summary">
                        @if ($detail->discount_on_food > 0)
                            Discount: -{{ number_format($discountAmount, 3) }} <span class="currency">(ر.ع)</span><br>
                        @endif

                        Addons: {{ number_format($itemAddOnsCost, 3) }} <span class="currency">(ر.ع)</span><br>
                        Total: {{ number_format($finalItemTotal, 3) }} <span class="currency">(ر.ع)</span>
                    </td>
                </tr>
    ...
    <div class="info-row">
        <div class="label">Items Price | <span class="arabic">سعر العناصر</span> <span class="currency">(ر.ع)</span></div>
        <div class="value">{{ number_format($subTotal, 3) }}</div>
    </div>

    <div class="info-row">
        <div class="label">Add-ons | <span class="arabic">الإضافات</span> <span class="currency">(ر.ع)</span></div>
        <div class="value">{{ number_format($addOnsCost, 3) }}</div>
    </div>

    <div class="info-row">
        <div class="label">Subtotal | <span class="arabic">المجموع الفرعي</span> <span class="currency">(ر.ع)</span></div>
        <div class="value">{{ number_format($subTotalWithAddons, 3) }}</div>
    </div>

    @if ($order->restaurant_discount_amount > 0)
        <div class="info-row">
            <div class="label">Discount On Bill | <span class="arabic">خصم على الفاتورة</span> <span class="currency">(ر.ع)</span></div>
            <div class="value">{{ number_format($order->restaurant_discount_amount, 3) }}</div>
        </div>
    @endif

    @if ($order->tax_status == 'excluded' || $order->tax_status == null)
        <div class="info-row">
            <div class="label">Tax | <span class="arabic">ضريبة</span> <span class="currency">(ر.ع)</span></div>
            <div class="value">{{ number_format($order->total_tax_amount, 3) }}</div>
        </div>
    @endif

    <div class="info-row">
        <div class="label">Delivery | <span class="arabic">توصيل</span> <span class="currency">(ر.ع)</span></div>
        <div class="value">{{ number_format($order->delivery_charge, 3) }}</div>
    </div>

    @if ($order->additional_charge > 0)
        <div class="info-row">
            <div class="label">Additional | <span class="arabic">إضافي</span> <span class="currency">(ر.ع)</span></div>
            <div class="value">{{ number_format($order->additional_charge, 3) }}</div>
        </div>
    @endif
    ...
    <div class="info-row medium">
        <div class="label">TOTAL | <span class="arabic">المجموع</span> <span class="currency">(ر.ع)</span></div>
        <div class="value">{{ number_format($order->order_amount, 3) }}</div>
    </div>
    ...
    @if ($order->pos_details)
        <div class="info-row">
            <div class="label">Cash | <span class="arabic">نقدي</span> <span class="currency">(ر.ع)</span></div>
            <div class="value">{{ number_format($order->pos_details->cash_paid, 3) }}</div>
        </div>

        <div class="info-row">
            <div class="label">Card | <span class="arabic">بطاقة</span> <span class="currency">(ر.ع)</span></div>
            <div class="value">{{ number_format($order->pos_details->card_paid, 3) }}</div>
        </div>
        ...
        @if ($change > 0)
            <div class="info-row">
                <div class="label">Change | <span class="arabic">يتغير</span> <span class="currency">(ر.ع)</span></div>
                <div class="value">{{ number_format($change, 3) }}</div>
            </div>
        @endif
    @endif
