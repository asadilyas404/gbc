...
@section('content')
                            ...
                            <div class="card">
                                <div class="card-body">
                                    <div class="d-flex justify-content-between">
                                        <div>
                                            <p><strong>{{!empty($order->cutomer) ? $order->cutomer->name : "Walk-in-Customer"}}</strong></p>
                                            <p><strong>Order Type:</strong> {{isset($data['order_type'][$order->order_type]) ? $data['order_type'][$order->order_type] : ""}}</p>
                                        </div>
                                        <div class="text-right">
                                            <p><strong>Order No:</strong> {{$order->id}}</p>
                                            <p><strong>Amount:</strong> {{$order->order_amount}} </p>
                                            @if($order->kitchen_time)
                                            <p class="timer" data-time="{{$order->kitchen_time}}">Time: {{$order->kitchen_time}}</p>
                                            @endif
                                        </div>
                                    </div>
                                    <div class="d-flex justify-content-between">
                                        <div>
                                            <a href="/restaurant-panel/order/details/{{$order->id}}" target="_blank" class="btn btn-primary btn-sm btn-style">Order Detail</a>
                                        </div>
                                        <div class="text-right">
                                            <button class="btn btn-primary btn-sm btn-style startCooking" data-id="{{$order->id}}">Start Cooking</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <div class="text-right">
                                                <button class="btn btn-primary btn-sm btn-style orderReady" data-id="{{$order->id}}">Ready</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="card">
                                    <div class="card-body">
                                        ...
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <div class="text-right">
                                                <button class="btn btn-primary btn-sm btn-style orderCompleted" data-id="{{$order->id}}">Completed</button>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <div class="toast" role="alert" aria-live="assertive" aria-atomic="true" data-delay="2000" style="position: absolute; top: 1rem; right: 1rem;z-index:9999">
                    ...
                    if(res.success){
                        ...
                        if(pendingList){
                            ...
                            Object.entries(pendingList).forEach(([id, item]) => {
                                let customer = (item.customer && item.customer?.name) ? item.customer?.name : "Walk-in-Customer"
                                pendingCard += funViewCard(customer, item, 'Start Cooking', 'startCooking');
                            });
        ...
        function updateTimers() {
            ...
            timerElements.forEach(timerElement => {
                ...
                function getKarachiTime() {
                    return new Date(new Date().toLocaleString("en-US", { timeZone: "{{$setTimezone}}" }));
                }
