...
@php($user->id = 0)
@if($user)
    @php($unchecked= isset($admin_conversation->last_message->sender_id) && ($admin_conversation->last_message->sender_id == $user->id) ? $admin_conversation->unread_message_count : 0)
    <div
        ...
        <div class="chat-user-info-content">
            <h5 class="mb-0 d-flex justify-content-between">
                ...
                @if (isset($admin_conversation->last_message->created_at))
                    <small>
            ...
            <div class="text-title">{{ isset($admin_conversation->last_message->message) ? Str::limit($admin_conversation->last_message->message ??'', 35, '...') : (count($admin_conversation->last_message->file_full_url ?? []) > 0 ?  count($admin_conversation->last_message->file_full_url) .' '. translate('messages.Attachments') :'' )}}</div>
        </div>
    </div>
@endif
